...
function getProviderLabel(firebaseUid: string, email: string): string {
  ...
  // Google UIDs are numeric strings; Microsoft UIDs contain alphanumeric chars.
  // More reliably, we look at the current Firebase user's providerData at runtime.
  // Since we only have the stored user here, we derive a best-effort label.
  // The UID format: Google = long numeric, Microsoft = GUID-like with dashes.
  if (/^\d+$/.test(firebaseUid)) return 'Google';
  if (/^[0-9a-f-]{30,}$/i.test(firebaseUid) && firebaseUid.includes('-')) return 'Microsoft';
  // email/password UIDs are 28-char alphanumeric strings from Firebase
  return 'Email';
}
...
function ProviderBadge({ firebaseUid, email }: { firebaseUid: string; email: string }) {
  ...
  const styles: Record<string, string> = {
    ...
    Email: 'bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400',
  };
  return (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-white/10 p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="flex gap-1 flex-wrap">
              {user.roles.map(r => (
                <span key={r} className="px-2 py-0.5 bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded text-xs">{r}</span>
              ))}
            </div>
          </div>
        </div>
