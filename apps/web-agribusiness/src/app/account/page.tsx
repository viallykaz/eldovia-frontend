...
function ProviderBadge({ firebaseUid, email }: { firebaseUid: string; email: string }) {
  ...
  const styles: Record<string, string> = {
    ...
    Email: 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400',
  };
  return (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-white/10 p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <div className="flex gap-1 flex-wrap">
              {user.roles.map(r => (
                <span key={r} className="px-2 py-0.5 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded text-xs">{r}</span>
              ))}
            </div>
          </div>
        </div>
